...
void sleeping(int index)
{
	...
	if (index % 2 == 0)
		printf("%d: filosofo %d esta durmiendo\n", current_time.tv_usec, index);
	else
		printf("%d: filosofo %d no puede dormir\n", current_time.tv_usec, index);
	usleep(5);
	if (index % 2 == 0)
		printf("%d: filosofo %d despertó\n", current_time.tv_usec, index);
	else
		printf("%d: filosofo %d se quedó dormido\n", current_time.tv_usec, index);
}
...
void *action(void *arg)
{
	...
	printf("%d :Un filosofo esta en la posición %d\n", current_time.tv_usec, index);
	usleep(8);
	sleeping(index);
	free(arg);
	return (NULL);
